...
- `0.4.0`：独立移动 App 验证版。以音乐场景验证 Tauri Mobile/PWA 技术路线、移动原生能力和真实设备体验，同时建立可供动漫、书籍、影视等领域复用的配置化评分内核，为 `1.0.0` 做准备；不在本版提前发布完整多领域 UI。详见[开发计划](./doc_CN/36_V0.4.0_独立移动App与多领域内核验证计划.md)。
- `1.0.0`：`XDRate` 多领域评分器，首要候选领域为音乐、动漫、书籍和影视；最终领域名单、默认轴、Schema 与首发平台须在 v0.4.0 验证后冻结。
