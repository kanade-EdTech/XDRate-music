...
- `0.3.1 Bili Toy`：当前评分卡和离线能力已适配为 B 站 Toy；独立构建、子路径资源审计、`toy_doctor.py` 预检和 CLI 预览通过，现有 `XDRateMusic` 版本已提交审核。详见[Bili Toy 适配开发计划](./doc_CN/37_V0.3.1_BiliToy_当前版本适配开发计划.md)。
...
- `0.5.0`：游戏评分试验版，用最小领域包和 3–5 个带封面的个人评价示例模板验证游戏评分规则、字段和卡片需求。详见[游戏评分试验开发计划](./doc_CN/42_V0.5.0_游戏评分试验开发计划.md)。
...
游戏评分试验入口：在本地地址后加 `?domain=game`。它与音乐主线隔离，目前用于模板和领域契约验证。
...
## Bili Toy v0.3.1 适配状态

已有评分卡已使用独立输出目录完成 Toy 子路径审计和 `toy_doctor.py` 预检，并提交到 `XDRateMusic` 条目审核。后续以审核结果和线上冒烟测试为准，不把 Toy 审核中的状态误写成公开发布。详见[Bili Toy 适配开发计划](./doc_CN/37_V0.3.1_BiliToy_当前版本适配开发计划.md)。
