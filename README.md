...
- `0.3.1`：在不改动 v0.3.0 候选包的前提下增加用户主动触发的小红书原生发帖联动；M2 已完成确认页、字符限制、隐私说明、调用前持久化与取消恢复，仍须完成 M3/M4 闭环和真机门禁。详见[联动开发计划](./doc_CN/31_V0.3.1_小红书帖子发布联动开发计划.md)和[M2 执行记录](./doc_CN/33_V0.3.1_M2_发布确认与恢复执行记录.md)。
