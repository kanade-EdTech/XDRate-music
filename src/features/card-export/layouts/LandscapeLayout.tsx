...
export function LandscapeLayout({
  ...
  const isCompact = config.ratio === '16:9';
  const hasNarrative = layoutPlan.hasNarrative;

  return (
    <div
      className="flex h-full w-full flex-col overflow-hidden"
      style={{ padding: isCompact ? '32px 40px' : '40px 48px', gap: isCompact ? '12px' : '16px' }}
    >
      <div className="grid min-h-0 flex-1 grid-cols-12 gap-6">
        {/* Left Column: Header + Score + (Narrative if present) */}
        <div
        ...
        >
          <div className="flex flex-col gap-4">
            <CardHeader work={draft.work} dark={dark} compact={isCompact} />
            <CardScoreBadge draft={draft} rating={rating} dark={dark} compact={isCompact} />
          </div>
        ...
        {/* Right Column: Chart + Breakdown/Reasons */}
        <div
          className={`col-span-6 flex min-h-0 flex-col gap-3 overflow-hidden ${
            hasNarrative ? 'justify-between' : 'justify-center'
          }`}
        >
          <div
            className="min-h-0 flex-1 overflow-hidden"
            style={{ flexGrow: layoutPlan.chartScale }}
          >
